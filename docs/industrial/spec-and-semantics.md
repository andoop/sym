...
- **成文清单**：[../spec/VM_SUBSET.md](../spec/VM_SUBSET.md)。
- 摘要：字面量、局部、`if`、`let`、块、`while`、算术与比较（含 `CompareVal`）、短路逻辑、标识符直接调用与用户 `fn`；内建 `println`/`eprintln`、`concat`、`string_from_int`、`strlen`、`exit`。
- **不** 在 VM：`match`、枚举构造、间接调用（见 `bytecode.rs`）。
